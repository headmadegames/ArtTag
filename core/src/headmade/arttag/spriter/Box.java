...
/**
 * Represents a box, which consists of four points: top-left, top-right, bottom-left and bottom-right. A box is responsible for checking
 * collisions and calculating a bounding box for a {@link Timeline.Key.Bone}.
 * 
 * @author Trixt0r
 *
 */
public class Box {
	 ...
	 *             if info or boneOrObject is <code>null</code>
	 */
	public void calcFor(Timeline.Key.Bone boneOrObject, ObjectInfo info) {
		final float width = info.size.width * boneOrObject.scale.x;
		final float height = info.size.height * boneOrObject.scale.y;

		final float pivotX = width * boneOrObject.pivot.x;
		final float pivotY = height * boneOrObject.pivot.y;
	 ...
	 *            the bone or object
	 * @param info
	 *            the object info of the given bone or object
	 * @param x
	 *            the x coordinate
	 * @param y
	 *            the y coordinate
	...
	public boolean collides(Timeline.Key.Bone boneOrObject, ObjectInfo info, float x, float y) {
		...
		final Point point = new Point(x - boneOrObject.position.x, y - boneOrObject.position.y);
