...
/**
 * Represents an entity of a Spriter SCML file. An entity holds {@link Animation}s, an {@link #id}, a {@link #name}. {@link #characterMaps}
 * and {@link #objectInfos} may be empty.
 * 
 * @author Trixt0r
 *
 */
public class Entity {
	...
	public boolean containsAnimation(Animation anim) {
		for (final Animation a : this.animations) {
			if (a == anim) {
				return true;
			}
		}
	...
	public Animation getAnimationWithMostTimelines() {
		...
		for (final Animation anim : this.animations) {
	...
	public CharacterMap getCharacterMap(String name) {
		for (final CharacterMap map : this.characterMaps) {
	 ...
	 * 
	 * @param name
	 *            the name of the object info
	 * @param type
	 *            the type if the object info
	...
	public ObjectInfo getInfo(String name, ObjectType type) {
		final ObjectInfo info = this.getInfo(name);
	...
	/**
	 * Represents the object info in a Spriter SCML file. An object info holds a {@link #type} and a {@link #name}. If the type is a Sprite
	 * it holds a list of frames. Otherwise it has a {@link #size} for debug drawing purposes.
	 * 
	 * @author Trixt0r
	 *
	 */
	...
	public String toString() {
		...
		toReturn += "Animations:\n";
		for (final Animation animaton : this.animations) {
			toReturn += "\n" + animaton;
		}
		toReturn += "]";
		return toReturn;
