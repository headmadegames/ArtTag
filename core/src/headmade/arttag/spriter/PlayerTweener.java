...
public class PlayerTweener extends Player {
	...
	/**
	 * Indicates whether to update the {@link Player} instances this instance is holding. If this variable is set to <code>false</code>, you
	 * will have to call {@link Player#update()} on your own.
	 */
	 ...
	 * 
	 * @param entity
	 *            the entity the players will animate
	 */
	 ...
	 *             if no bone with {@link #baseBoneName} exists
	 */
	@Override
	public void update() {
		if (updatePlayers) {
		...
		if (baseBoneName != null) {
			final int index = anim.onFirstMainLine() ? player1.getBoneIndex(baseBoneName) : player2.getBoneIndex(baseBoneName);
			if (index == -1) {
	 ...
	 * 
	 * @param index
	 *            the index of the base animation
	 */
	...
	/**
	 * Sets the base animation of this tweener by the given name. Has only an effect if {@link #baseBoneName} is not <code>null</code>.
	 * 
	 * @param name
	 *            the name of the base animation
	 */
