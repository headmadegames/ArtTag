...
import headmade.arttag.spriter.Mainline.Key;
...
/**
 * Represents an animation of a Spriter SCML file. An animation holds {@link Timeline}s and a {@link Mainline} to animate objects.
...
public class Animation {
	 ...
	 *
	 * @param time
	 *            The time which has to be between 0 and {@link #length} to work properly.
	 * @param root
	...
	public void update(int time, Bone root) {
		...
		for (final ObjectRef ref : currentKey.objectRefs) {
			this.update(ref, root, time);
		}
	}
	...
	protected void update(BoneRef ref, Bone root, int time) {
		...
		final Timeline timeline = getTimeline(ref.timeline);
		final Timeline.Key key = timeline.getKey(ref.key);
		Timeline.Key nextKey = timeline.getKey((ref.key + 1) % timeline.keys.length);
		final int currentTime = key.time;
		...
		if (currentKey.time > currentTime) {
			float tMid = (float) (currentKey.time - currentTime) / (nextTime - currentTime);
		...
		// Tween bone/object
		final Bone bone1 = key.object();
		final Bone bone2 = nextKey.object();
		final Bone tweenTarget = this.tweenedKeys[ref.timeline].object();
		if (isObject) {
	...
	void unmapTimelineObject(int timeline, boolean isObject, Bone root) {
		final Bone tweenTarget = this.tweenedKeys[timeline].object();
		final Bone mapTarget = this.unmappedTweenedKeys[timeline].object();
		if (isObject) {
	...
	/*
	 * Timeline getSimilarTimeline(BoneRef ref, Collection<Timeline> coveredTimelines){ if(ref.parent == null) return null; for(BoneRef
	 * boneRef: this.currentKey.objectRefs){ Timeline t = this.getTimeline(boneRef.timeline); if(boneRef.parent != null && boneRef.parent.id
	 * == ref.parent.id && !coveredTimelines.contains(t)) return t; } return null; }
	 *
	 * Timeline getSimilarTimeline(ObjectRef ref, Collection<Timeline> coveredTimelines){ if(ref.parent == null) return null; for(ObjectRef
	 * objRef: this.currentKey.objectRefs){ Timeline t = this.getTimeline(objRef.timeline); if(objRef.parent != null && objRef.parent.id ==
	 * ref.parent.id && !coveredTimelines.contains(t)) return t; } return null; }
	 */

	/**
	 * Prepares this animation to set this animation in any time state. This method has to be called before {@link #update(int, Bone)}.
	 */
	public void prepare() {
		if (this.prepared) {
			return;
