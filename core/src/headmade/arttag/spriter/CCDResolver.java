...
/**
 * An inverse kinematics resolver implementation. An instance of this class uses the CCD (Cyclic Coordinate Descent) algorithm to resolve
 * the constraints.
 * 
...
public class CCDResolver extends IKResolver {
	...
	public void resolve(float x, float y, int chainLength, BoneRef effectorRef) {
		...
		final Timeline timeline = player.animation.getTimeline(effectorRef.timeline);
		final Timeline.Key key = player.tweenedKeys[effectorRef.timeline];
		final Timeline.Key unmappedKey = player.unmappedTweenedKeys[effectorRef.timeline];
		final Bone effector = key.object();
		final Bone unmappedffector = unmappedKey.object();
