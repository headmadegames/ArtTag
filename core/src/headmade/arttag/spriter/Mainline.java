 ...
 * responsible for telling which draw order the sprites have and how the objects are related to each other, i.e. which bone is the root and
 * which objects are the children.
 * 
 * @author Trixt0r
 *
 */
public class Mainline {
	 ...
	 * 
	 * @param time
	 *            the time a key has to be before
	...
	/**
	 * Represents a mainline key in a Spriter SCML file. A mainline key holds an {@link #id}, a {@link #time}, a {@link #curve} and lists of
	 * bone and object references which build a tree hierarchy.
	 * 
	 * @author Trixt0r
	 *
	 */
	public static class Key {
		 ...
		 * 
		 * @param index
		 *            the index of the bone reference
		 ...
		 * 
		 * @param index
		 *            the index of the object reference
		 ...
		 * 
		 * @param ref
		 *            the reference to the reference in this key
		...
		public BoneRef getBoneRefTimeline(int timeline) {
			for (final BoneRef boneRef : this.boneRefs) {
		...
		public ObjectRef getObjectRefTimeline(int timeline) {
			for (final ObjectRef objRef : this.objectRefs) {
		...
		public String toString() {
			...
			for (final BoneRef ref : boneRefs) {
				toReturn += "\n" + ref;
			}
			for (final ObjectRef ref : objectRefs) {
				toReturn += "\n" + ref;
			}
			toReturn += "]";
			return toReturn;
		...
		/**
		 * Represents a bone reference in a Spriter SCML file. A bone reference holds an {@link #id}, a {@link #timeline} and a {@link #key}
		...
		public static class BoneRef {
			...
			@Override
			public String toString() {
				final int parentId = parent != null ? parent.id : -1;
				return getClass().getSimpleName() + "|id: " + id + ", parent:" + parentId + ", timeline: " + timeline + ", key: " + key;
			}
		}

		/**
		 * Represents an object reference in a Spriter SCML file. An object reference extends a {@link BoneRef} with a {@link #zIndex},
