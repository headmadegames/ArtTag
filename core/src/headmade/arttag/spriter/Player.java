...
import headmade.arttag.spriter.Entity.CharacterMap;
import headmade.arttag.spriter.Entity.ObjectInfo;
import headmade.arttag.spriter.Mainline.Key.BoneRef;
import headmade.arttag.spriter.Mainline.Key.ObjectRef;
import headmade.arttag.spriter.Timeline.Key.Bone;
import headmade.arttag.spriter.Timeline.Key.Object;

/**
 * A Player instance is responsible for updating an {@link Animation} properly. With the {@link #update()} method an instance of this class
 * will increase its current time and update the current set animation ({@link #setAnimation(Animation)}). A Player can be positioned with
 * {@link #setPivot(float, float)}, scaled with {@link #setScale(float)}, flipped with {@link #flip(boolean, boolean)} and rotated
 * {@link #setAngle(float)}. A Player has various methods for runtime object manipulation such as {@link #setBone(String, Bone)} or
 * {@link #setObject(String, Bone)}. Events like the ending of an animation can be observed with the {@link PlayerListener} interface.
 * Character maps can be changed on the fly, just by assigning a character maps to {@link #characterMaps}, setting it to <code>null</code>
...
public class Player {
	 ...
	 *
	 * @param entity
	 *            the entity this player will animate
	 */
	...
	/**
	 * Updates this player. This means the current time gets increased by {@link #speed} and is applied to the current animation.
	 */
	public void update() {
	...
	public int getBoneIndex(String name) {
		for (final BoneRef ref : getCurrentKey().boneRefs) {
	 ...
	 *             if no bone exists with the given name
	 ...
	 * @throws NullPointerException
	 *             if no reference for the given bone was found
	 */
	...
	public int getObjectIndex(String name) {
		for (final ObjectRef ref : getCurrentKey().objectRefs) {
			if (animation.getTimeline(ref.timeline).name.equals(name)) {
				return ref.id;
			}
		}
	 ...
	 * @throws NullPointerException
	 *             if no reference for the given object was found
	 */
	 ...
	 * @throws NullPointerException
	 *             if no name for the given bone or bject was found
	 */
	 ...
	 * @throws NullPointerException
	 *             if no object info for the given bone or bject was found
	 */
	 ...
	 *
	 * @param boneOrObject
	 *            the bone or object to calculate the bounding box for
	 * @return the box for the given bone or object
	 * @throws NullPointerException
	 *             if no object info for the given bone or object exists
	 */
	public Box getBox(Bone boneOrObject) {
		final ObjectInfo info = getObjectInfoFor(boneOrObject);
	 ...
	 *            the name of the bone
	 * @param x
	 *            the new x value of the bone
	 * @param y
	 *            the new y value of the bone
	 * @param angle
	 *            the new angle of the bone
	 * @param scaleX
	 *            the new scale in x direction of the bone
	 * @param scaleY
	 *            the new scale in y direction of the bone
	 * @throws SpriterException
	 *             if no bone exists of the given name
	 */
	public void setBone(String name, float x, float y, float angle, float scaleX, float scaleY) {
		final int index = getBoneIndex(name);
		if (index == -1) {
			throw new SpriterException("No bone found of name \"" + name + "\"");
		}
		final BoneRef ref = getCurrentKey().getBoneRef(index);
		final Bone bone = getBone(index);
	 ...
	 *            the name of the object
	 * @param x
	 *            the new position in x direction of the object
	 * @param y
	 *            the new position in y direction of the object
	 * @param angle
	 ...
	 * @param pivotY
	 *            the new pivot in y direction of the object
	 * @param alpha
	 *            the new alpha value of the object
	 * @param folder
	 *            the new folder index of the object
	 * @param file
	 *            the new file index of the object
	 * @throws SpriterException
	 *             if no object exists of the given name
	 */
	public void setObject(String name, float x, float y, float angle, float scaleX, float scaleY, float pivotX, float pivotY, float alpha,
			int folder, int file) {
		final int index = getObjectIndex(name);
		if (index == -1) {
			throw new SpriterException("No object found for name \"" + name + "\"");
		}
		final ObjectRef ref = getCurrentKey().getObjectRef(index);
		final Object object = getObject(index);
	 ...
	 * @param pivot
	 *            the new pivot of the object
	 * @param alpha
	 *            the new alpha value of the object
	 * @param ref
	 *            the new file reference of the object
	 * @throws SpriterException
	 *             if no object exists of the given name
	 */
	 ...
	 *
	 * @param base
	 *            the root bone to start at. Set it to <code>null</code> to traverse the whole bone hierarchy.
	 */
	public void unmapObjects(BoneRef base) {
		final int start = base == null ? -1 : base.id - 1;
		for (int i = start + 1; i < getCurrentKey().boneRefs.length; i++) {
			final BoneRef ref = getCurrentKey().getBoneRef(i);
			if (ref.parent != base && base != null) {
				continue;
			}
			final Bone parent = ref.parent == null ? this.root : this.unmappedTweenedKeys[ref.parent.timeline].object();
	...
	public void setEntity(Entity entity) {
		...
		this.entity = entity;
		final int maxAnims = entity.getAnimationWithMostTimelines().timelines();
		tweenedKeys = new Timeline.Key[maxAnims];
		unmappedTweenedKeys = new Timeline.Key[maxAnims];
		for (int i = 0; i < maxAnims; i++) {
			final Timeline.Key key = new Timeline.Key(i);
			final Timeline.Key keyU = new Timeline.Key(i);
	 ...
	 *            the new animation
	 * @throws SpriterException
	 *             if the animation is <code>null</code> or the current animation is not a member of the current set entity
	 */
	public void setAnimation(Animation animation) {
		final Animation prevAnim = this.animation;
	 ...
	 * @throws SpriterException
	 *             if no animation exists with the given name
	 */
	...
	/**
	 * Returns a bounding box for this player. The bounding box is calculated for all bones and object starting from the given root.
	 *
	 * @param root
	 *            the starting root. Set it to null to calculate the bounding box for the whole player
	 * @return the bounding box
	 */
	public Rectangle getBoundingRectangle(BoneRef root) {
		final Bone boneRoot = root == null ? this.root : this.unmappedTweenedKeys[root.timeline].object();
	...
	private void calcBoundingRectangle(BoneRef root) {
		for (final BoneRef ref : getCurrentKey().boneRefs) {
			...
			final Bone bone = this.unmappedTweenedKeys[ref.timeline].object();
	...
	/**
	 * Sets the time for the current time. The player will make sure that the new time will not exceed the time bounds of the current
	 * animation.
	 *
	 * @param time
	 *            the new time
	 ...
	 *
	 * @param scale
	 *            the new scale. 1f means 100% scale.
	 * @return this player to enable chained operations
	 */
	 ...
	 *
	 * @param scale
	 *            the scaling factor. 1f means no scale.
	 * @return this player to enable chained operations
	 */
	 ...
	 *
	 * @param x
	 *            whether to flip the player around the x axis
	 * @param y
	 *            whether to flip the player around the y axis
	 * @return this player to enable chained operations
	 */
	 ...
	 *
	 * @param x
	 *            the new position in x direction
	 * @param y
	 *            the new position in y direction
	 * @return this player to enable chained operations
	 */
	...
	/**
	 * Sets the pivot, i.e. origin, of this player. A pivot at (0,0) means that the origin of the played animation will have the same one as
	 * in Spriter.
	 *
	 * @param x
	 *            the new pivot in x direction
	 * @param y
	 ...
	 *
	 * @param start
	 *            the bone reference to start at
	 * @return the bone iterator
	 */
	 ...
	 *
	 * @param start
	 *            the object reference to start at
	 * @return the object iterator
	 */
	...
	public static interface PlayerListener {
		...
		/**
		 * Gets called if the animation of the player gets changed. If {@link Player#setAnimation(Animation)} gets called and the new
		 * animation is the same as the previous one, this method will not be called.
		 *
		 * @param oldAnim
		 *            the old animation
		 * @param newAnim
		...
		/**
		 * Gets called if the mainline key gets changed. If {@link Player#speed} is big enough it can happen that mainline keys between the
		 * previous and the new mainline key will be ignored.
		 *
		 * @param prevKey
	...
	/**
	 * An attachment is an abstract object which can be attached to a {@link Player} object. An attachment extends a {@link Bone} which
	 * means that {@link Bone#position}, {@link Bone#scale} and {@link Bone#angle} can be set to change the relative position to its
