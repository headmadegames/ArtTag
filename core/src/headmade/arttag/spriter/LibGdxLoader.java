...
public class LibGdxLoader extends Loader<Sprite>implements Disposable {
	...
	private final HashMap<FileReference, Pixmap>	pixmaps;
	private final HashMap<Pixmap, Boolean>			pixmapsToDispose;
	...
	protected Sprite loadResource(FileReference ref) {
		FileHandle f;
		final String path = super.root + "/" + data.getFile(ref).name;
	...
	protected void generatePackedSprites() {
		...
		final TextureAtlas tex = this.packer.generateTextureAtlas(TextureFilter.Linear, TextureFilter.Linear, false);
		final Set<FileReference> keys = this.resources.keySet();
		this.disposeNonPackedTextures();
		for (final FileReference ref : keys) {
			final TextureRegion texReg = tex.findRegion(data.getFile(ref).name);
	...
	private void disposeNonPackedTextures() {
		for (final Entry<FileReference, Sprite> entry : super.resources.entrySet()) {
	...
	protected void finishLoading() {
		final Set<FileReference> refs = this.resources.keySet();
		for (final FileReference ref : refs) {
			final Pixmap pix = this.pixmaps.get(ref);
	...
	protected void createSprite(FileReference ref, Pixmap image) {
		final Texture tex = new Texture(image);
		tex.setFilter(TextureFilter.Linear, TextureFilter.Linear);
		final int width = (int) data.getFile(ref.folder, ref.file).size.width;
		final int height = (int) data.getFile(ref.folder, ref.file).size.height;
		final TextureRegion texRegion = new TextureRegion(tex, width, height);
		super.resources.put(ref, new Sprite(texRegion));
		pixmapsToDispose.put(image, true);
	}

	protected void disposePixmaps() {
		final Pixmap[] maps = new Pixmap[this.pixmapsToDispose.size()];
		this.pixmapsToDispose.keySet().toArray(maps);
		for (final Pixmap pix : maps) {
			try {
