...
/**
 * A utility class to keep the code short. A point is essentially that what you would expect if you think about a point in a 2D space. It
 * holds an x and y value. You can {@link #translate(Point)}, {@link #scale(Point)}, {@link #rotate(float)} and {@link #set(Point)} a point.
 * 
 * @author Trixt0r
 *
 */
public class Point {
	 ...
	 * 
	 * @param point
	 *            the point to set this point at
	 */
	 ...
	 * 
	 * @param x
	 *            the amount in x direction to add
	 * @param y
	 *            the amount in y direction to add
	 * @return this point for chained operations
	 */
	 ...
	 * 
	 * @param x
	 *            the scale amount in x direction
	 * @param y
	 *            the scale amount in y direction
	 * @return this point for chained operations
	 */
	...
	public Point rotate(float degrees) {
		if (x != 0 || y != 0) {
			final float cos = Calculator.cosDeg(degrees);
			final float sin = Calculator.sinDeg(degrees);

			final float xx = x * cos - y * sin;
			final float yy = x * sin + y * cos;
