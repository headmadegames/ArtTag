...
import static headmade.arttag.spriter.Calculator.solveCubic;
import static headmade.arttag.spriter.Interpolator.bezier;
import static headmade.arttag.spriter.Interpolator.cubic;
import static headmade.arttag.spriter.Interpolator.cubicAngle;
import static headmade.arttag.spriter.Interpolator.linear;
import static headmade.arttag.spriter.Interpolator.linearAngle;
import static headmade.arttag.spriter.Interpolator.quadratic;
import static headmade.arttag.spriter.Interpolator.quadraticAngle;
import static headmade.arttag.spriter.Interpolator.quartic;
import static headmade.arttag.spriter.Interpolator.quarticAngle;
import static headmade.arttag.spriter.Interpolator.quintic;
import static headmade.arttag.spriter.Interpolator.quinticAngle;

/**
 * Represents a curve in a Spriter SCML file. An instance of this class is responsible for tweening given data. The most important method of
 * this class is {@link #tween(float, float, float)}. Curves can be changed with sub curves {@link Curve#subCurve}.
 *
 * @author Trixt0r
 *
 */
public class Curve {
	 ...
	 *            the curve type
	 * @param subCurve
	 *            the sub curve. Can be <code>null</code>
	 */
	 ...
	 *            the end value
	 * @param t
	 *            the weight which lies between 0.0 and 1.0
	...
	public float tween(float a, float b, float t) {
		...
		case Bezier:
			Float cubicSolution = solveCubic(3f * (constraints.c1 - constraints.c3) + 1f, 3f * (constraints.c3 - 2f * constraints.c1),
					3f * constraints.c1, -t);
	 ...
	 *            the weight which lies between 0.0 and 1.0
	 * @param target
	 *            the target point to save the result in
	 */
	 ...
	 *            the weight which lies between 0.0 and 1.0
	 * @param spin
	 *            the spin, which is either 0, 1 or -1
	...
	public float tweenAngle(float a, float b, float t) {
		...
		case Quartic:
			return quarticAngle(a, linearAngle(a, b, constraints.c1), linearAngle(a, b, constraints.c2), linearAngle(a, b, constraints.c3),
					b, t);
		case Quintic:
			return quinticAngle(a, linearAngle(a, b, constraints.c1), linearAngle(a, b, constraints.c2), linearAngle(a, b, constraints.c3),
