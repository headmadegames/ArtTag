...
/**
 * A Drawer is responsible for drawing a {@link Player}. Since this library is meant to be as generic as possible this class has to be
 * abstract, because it cannot be assumed how to draw a resource. Anyone who wants to draw a {@link Player} has to know how to draw a
 * resource. A resource can be e.g. a sprite, a texture or a texture region. To draw a {@link Player} call {@link #draw(Player)}. This
 * method relies on {@link #draw(Object)}, which has to be implemented with the chosen backend. To debug draw a {@link Player} call
 * {@link #drawBones(Player)}, {@link #drawBoxes(Player)} and {@link #drawPoints(Player)}, which rely on
 * {@link #rectangle(float, float, float, float)}, {@link #circle(float, float, float)}, {@link #line(float, float, float, float)} and
 * {@link #setColor(float, float, float, float)}.
 *
 * @author Trixt0r
 *
 * @param <R>
 *            The backend specific resource. In general such a resource is called "sprite", "texture" or "image".
 */
public abstract class Drawer<R> {
	 ...
	 *             if the loader is <code>null</code>
	 */
	...
	public void drawBones(Player player) {
		...
		final Iterator<Bone> it = player.boneIterator();
		while (it.hasNext()) {
			final Timeline.Key.Bone bone = it.next();
			final Timeline.Key key = player.getKeyFor(bone);
			if (!key.active) {
				continue;
			}
			final ObjectInfo info = player.getObjectInfoFor(bone);
			final Dimension size = info.size;
			drawBone(bone, size);
		}
		/*
		 * for(Mainline.Key.BoneRef ref: player.getCurrentKey().boneRefs){ Timeline.Key key = player.unmappedTweenedKeys[ref.timeline];
		 * Timeline.Key.Bone bone = key.object(); if(player.animation.getTimeline(ref.timeline).objectInfo.type != ObjectType.Bone ||
		 * !key.active) continue; ObjectInfo info = player.animation.getTimeline(ref.timeline).objectInfo; if(info == null) continue;
		 * Dimension size = info.size; drawBone(bone, size); }
		 */
	}
	...
	public void drawBone(Bone bone, Dimension size) {
		...
		final float xx = bone.position.x + (float) Math.cos(Math.toRadians(bone.angle)) * size.height;
		final float yy = bone.position.y + (float) Math.sin(Math.toRadians(bone.angle)) * size.height;
		final float x2 = (float) Math.cos(Math.toRadians(bone.angle + 90)) * halfHeight * bone.scale.y;
		final float y2 = (float) Math.sin(Math.toRadians(bone.angle + 90)) * halfHeight * bone.scale.y;

		final float targetX = bone.position.x + (float) Math.cos(Math.toRadians(bone.angle)) * size.width * bone.scale.x,
				targetY = bone.position.y + (float) Math.sin(Math.toRadians(bone.angle)) * size.width * bone.scale.x;
		final float upperPointX = xx + x2, upperPointY = yy + y2;
		this.line(bone.position.x, bone.position.y, upperPointX, upperPointY);
		this.line(upperPointX, upperPointY, targetX, targetY);

		final float lowerPointX = xx - x2, lowerPointY = yy - y2;
	 ...
	 *
	 * @param player
	 *            the player to draw the boxes from
	 */
	 ...
	 *
	 * @param player
	 *            the player to draw the bone boxes of
	 */
	 ...
	 *            the player to draw the bone boxes of
	 * @param it
	 *            the iterator iterating over the bones to draw
	 */
	public void drawBoneBoxes(Player player, Iterator<Bone> it) {
		while (it.hasNext()) {
			final Bone bone = it.next();
	 ...
	 *
	 * @param player
	 *            player the player to draw the object boxes of
	 * @param it
	 *            the iterator iterating over the object to draw
	 */
	public void drawObjectBoxes(Player player, Iterator<Object> it) {
		while (it.hasNext()) {
			final Object bone = it.next();
			this.drawBox(player.getBox(bone));
		}
	}
	 ...
	 *
	 * @param player
	 *            the player to draw the points of.
	 */
	 ...
	 *
	 * @param player
	 *            player the player to draw the points of
	 * @param it
	 *            the iterator iterating over the points to draw
	 */
	public void drawPoints(Player player, Iterator<Object> it) {
		while (it.hasNext()) {
			final Object point = it.next();
			if (player.getObjectInfoFor(point).type == ObjectType.Point) {
				final float x = point.position.x + (float) (Math.cos(Math.toRadians(point.angle)) * pointRadius);
				final float y = point.position.y + (float) (Math.sin(Math.toRadians(point.angle)) * pointRadius);
	 ...
	 *
	 * @param it
	 *            the iterator iterating over the objects to draw
	 * @param map
	 *            the character map to draw
	 */
	public void draw(Iterator<Timeline.Key.Object> it, CharacterMap[] maps) {
		while (it.hasNext()) {
			final Timeline.Key.Object object = it.next();
			if (object.ref.hasFile()) {
				if (maps != null) {
					for (final CharacterMap map : maps) {
						if (map != null) {
	 ...
	 *
	 * @param r
	 *            the red value between 0.0 - 1.0
	 * @param g
	 *            the green value between 0.0 - 1.0
	 * @param b
	 *            the blue value between 0.0 - 1.0
	 * @param a
	 *            the alpha value between 0.0 - 1.0
	 */
